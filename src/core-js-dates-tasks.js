...
function getTime(date) {
  const getValuesOfDate = (val) => {
    return val >= 10 ? val : `0${val}`;
  };

  const hours = getValuesOfDate(date.getHours());
  const minutes = getValuesOfDate(date.getMinutes());
  const seconds = getValuesOfDate(date.getSeconds());
...
function getNextFriday(date) {
  ...
  const setNextDay = (val) => {
    return new Date(date.setDate(currentDay + val));
  };

  let step = 1;
  let nextFriday = setNextDay(step);

  while (nextFriday.getDay() !== 5) {
    step += 1;
    nextFriday = setNextDay(step);
  }
...
function getCountDaysInMonth(month, year) {
  ...
  const days = new Date(nextMonth - currentMonth).getDate();
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = (end - start) / msPerDay + 1;
...
function formatDate(date) {
  ...
  const getValuesOfTime = (val) => {
    return val >= 10 ? val : `0${val}`;
  };

  const minutes = getValuesOfTime(fullDate.getMinutes());
  const seconds = getValuesOfTime(fullDate.getSeconds());
  let hours = fullDate.getUTCHours();

  if (hours < 12) {
    result = `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} AM`;
  } else {
    if (hours > 12) {
      hours -= 12;
    }

    result = `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} PM`;
  }
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (counter <= lastDay) {
    ...
    counter += 1;
    firstDay = new Date(firstDay.setDate(counter));
  }
...
function getWeekNumberByDate(date) {
  ...
  const days = (date - yearStart) / 86400000;
  const weeks = Math.ceil((days + yearStart.getDay() + 1) / 7);
...
function getNextFridayThe13th(date) {
  ...
  let nextUnluckyFriday;

  while (year) {
    nextUnluckyFriday = new Date(year, month, 13);

    if (nextUnluckyFriday.getDay() === 5) {
      break;
    }
